...
let loanDuration = document.getElementById("loanPeriod");
...
function calculateEMI(){
    // First calculate total number of months in loan period if selected year
    let isYear = document.getElementById("year").checked;
    let isMonth = document.getElementById("month").checked;
    let noOfMonths=0;
    if(isYear=="" && isMonth==""){
        alert("Please select loan period type-> Month or year");
    }else{
        if(isYear==true){
